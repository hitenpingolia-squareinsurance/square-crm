...
@Component({
  selector: 'app-control-cases-online',
  templateUrl: './control-cases-online.component.html',
  styleUrls: ['./control-cases-online.component.css']
})

export class ControlCasesOnlineComponent implements OnInit {
 ...
 dom: 'ilpftripl',
      ajax: (dataTablesParameters: any, callback) => {
        that.http.post<DataTablesResponse>(this.api.additionParmsEnc(environment.apiUrl + "/Controlcases/QuotationDataFetch?User_Id=" + this.api.GetUserData("Id") +
  ...
  AddControlCase(Id,ControllId,QuotationId,LOB){
    ...
    alert(QuotationId);
      ...
      formData.append("loginType", this.api.GetUserType());
        ...
        formData.append("controll_id", ControllId);
        formData.append("qid", QuotationId);
        formData.append("LOB", LOB);

        this.api
        .HttpPostType("Controlcases/policy_details", formData)
        .then( (result) => {
          if (result["status"] == true) {
            ...
            if (GetUserType == 'employee') {
              ...
              // a.href = this.api.ReturnWebUrl() + '/Prequotes/SetSessionEmployee/login/' + btoa(UserDatas) + '?ReturnUrl=' + result["URL"];
            }
            else {
              ...
              // a.href = this.api.ReturnWebUrl() + '/agents/check/' + btoa(UserDatas) + '/' + btoa(GetUserType) + '/login?ReturnUrl=' + result["URL"];
