...
export class BusinessreportComponent implements OnInit {
        ...
        that.http
          .post<DataTablesResponse>(
            this.api.additionParmsEnc(
              environment.apiUrl +
                "/b-crm/customreports/PolicyDataFetch?User_Id=" +
  ...
  GetFromDate(ToDate, FromDate, GraphVal, val) {
    ...
    this.api
      .HttpPostType(
        "b-crm/customreports/PolicyDataFetchCustomReports",
        formData
      )
      .then(
        (result: any) => {
          ...
          if (result["Status"] == true) {
            this.GraphType = result["GraphType"];
            ...
            this.TotalToRevenue = result["TotalToRevenue"];
            this.TotalFromRevenue = result["TotalFromRevenue"];

            this.dataArValuee = result["TableValue"];

            this.GrowthCheckTotalNet = result["GrowthCheckTotalNet"];
            this.GrowthTotalNet = result["GrowthTotalNet"];
            ...
            var ToPospActivation = result["ToPospActivation"];
            var FromPospActivation = result["FromPospActivation"];

            this.FromPospActivation = FromPospActivation.split("/");
            this.ToPospActivation = ToPospActivation.split("/");

            this.GrowthTotalPospActivation =
              result["GrowthTotalPospActivation"];

            this.GrowthCheckTotalPospActivation =
              result["GrowthCheckTotalPospActivation"];

            this.TotalToPospActivation = result["TotalToPospActivation"];
            this.TotalFromPospActivation = result["TotalFromPospActivation"];

            if (result["GraphType"] != "Rm Wise") {
