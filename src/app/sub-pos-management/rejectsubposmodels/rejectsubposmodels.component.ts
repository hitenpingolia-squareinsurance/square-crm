...
@Component({
  selector: 'app-rejectsubposmodels',
  templateUrl: './rejectsubposmodels.component.html',
  styleUrls: ['./rejectsubposmodels.component.css']
})
export class RejectsubposmodelsComponent implements OnInit {
  ...
  constructor(public api : ApiService,private http: HttpClient,public dialogRef: MatDialogRef<RejectsubposmodelsComponent>,@Inject(MAT_DIALOG_DATA) public data: any,private fb: FormBuilder) {
  ...
  GetRejectedDiscription(Ids:number){
    ...
    this.api.HttpGetType('Subpos/GetRemarks/'+Ids).then((result:any) => {
      ...
      var Confirms=confirm('Are You Sure To Change Status?');
		if(Confirms==true){
			...
			this.api.IsLoading(); 
			 ...
			 this.api.HttpPostType('Subpos/UpdateActionProcessManger/Reject',formData).then((result:any) => {
