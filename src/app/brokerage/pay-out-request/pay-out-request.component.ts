...
//import { PayoutAgentGroupRequestViewComponent } from '../../modals/brokerage/payout-agent-group-request-view/payout-agent-group-request-view.component';
...
@Component({
  selector: "app-pay-out-request",
  templateUrl: "./pay-out-request.component.html",
  styleUrls: ["./pay-out-request.component.css"],
})
export class PayOutRequestComponent implements OnInit {
  ...
  SearchComponentsData() {
    ...
    this.api
      .Call(
        "pay/PayoutRMAuthority/SearchComponentsData?User_Id=" +
          this.api.GetUserId()
      )
      .then(
        (result) => {
          ...
          if (result["Status"] == true) {
            ...
            this.States_Ar = result["Data"]["State_Ar"];
            this.RTO_Ar = []; //result['Data']['RTO_Ar'];
            this.Products_Ar = result["Data"]["Products_Ar"];
            this.SubProducts_Ar = result["Data"]["SubProducts_Ar"];

            this.Segment_Ar = result["Data"]["Segments_Ar"];
            this.Classes_Ar = result["Data"]["Class_Ar"];
            this.Sub_Classes_Ar = result["Data"]["SubClass_Ar"];
          } else {
          ...
          .post<DataTablesResponse>(
            this.api.additionParmsEnc(
              environment.apiUrlBmsBase +
                "/pay/PayoutRMAuthority/AllPayOutRequest_GridData?User_Id=" +
                this.api.GetUserId() +
                "&source=crm"
            ),
  ...
  BlockRequest(Id) {
    var r = confirm("are you sure you want to block this pay-in request !");
    if (r == true) {
      //txt = "You pressed OK!";
      ...
      this.api
        .Call(
          "pay/PayIn/Block_RMAuthority_Request?Id=" +
            Id +
            "&User_Id=" +
            this.api.GetUserId()
        )
  ...
  GetAgents(item: any) {
    ...
    formData.append("RM_Id", item.Id);

    this.api.IsLoading();
    this.api.HttpPostType("pay/PayoutRMAuthority/GetAgents_New", formData).then(
      (result) => {
        this.api.HideLoading();
        if (result["Status"] == true) {
          this.PayInForm.get("Agent_Ids").setValue(null);
  ...
  ExportExcel(): void {
    ...
    dialogConfig.data = {
      ReportType: "RMPayoutRequestExport",
