...
type NewType = LostRenewalCasesComponent;

@Component({
  selector: 'app-lost-renewal-cases',
  templateUrl: './lost-renewal-cases.component.html',
  styleUrls: ['./lost-renewal-cases.component.css'],
  changeDetection: ChangeDetectionStrategy.Default,
})



export class LostRenewalCasesComponent implements OnInit {
  ...
  page: any;
  datas: any;
  NestedPagesListQuery: any;
  types_employee: any;
  LastPageId: number = 0;
  checkNoPagenoNested: number = 1;
  uniqueSet: Array<any> = [];
  NestedType: any;
  ...
  constructor(
    ...
    this.pageval = data.page;
    this.datas = data.datas;
    this.NestedPagesListQuery = data.NestedPagesListQuery;
    this.types_employee = data.Types;

    this.NestedType = data.NestedType ? data.NestedType : ""; //by new flag of lost renewal
  ...
  ngOnInit(): void {
    if (this.types_employee == "employee") this.SearchData1("");

    // $(".cdk-overlay-pane").css("width", "87% !important");//
  ...
  SearchData1(event: any) {
    this.filterData1 = JSON.stringify(event);
    this.post1 = [];
    this.pageNo1 = 1;
    this.LastPageId = 0;
    ...
    this.GetBulkCases(this.AgentName, this.AgentId, "click", 1);
  ...
  GetBulkCases(AgentName: string, AgentIds: string, types = "", page) {
    ...
    if (this.checkNoPagenoNested === 1 && !this.isLoadingFlag) {
      ...
      formData.append("NestedPagesListQuery", this.NestedPagesListQuery);

      // this.api.IsLoading();

      let urlsConditionally = "girdDataNested";
      if (this.NestedType == "lost") urlsConditionally = "girdDataNestedLost";
          ...
          (result: any) => {
            ...
            if (result["status"] == true) {
              ...
              if (result["data"].length > 0) {
                ...
                // console.log(result["data"],"postNested");


                this.postNested = this.postNested.concat(
                  this.responseDataNested
                );
                this.postNested = this.Removedatass(this.postNested);
                ...
                var pages = Math.ceil(this.postNested.length / 20);
                this.pageNoNested = pages + 1;
                //   //   //   console.log(this.postNested.length+' '+this.pageNoNested);
              } else {
                this.checkNoPagenoNested = 0;
              }
  ...
  ChangeStatusRenewals(e: any, Id: number, Action: string) {
    ...
    if (
      this.urlSegmentSub == "manage-renewal-lost-request" &&
      Values == "Renewed"
    ) {
      return false;
    }
    ...
    if (Values == "") {
      ...
      const dialogRef = this.dialog.open(RenewalfollowformComponent, {
        width: "60%",
        height: "60%",
        data: { Id: Id, Status: Values, Status2: Values2, ActionUser: "user",RenewalType:"lost" },
      });
