...
export class StatementsComponent implements OnInit {
  ...
  YearData: { Id: string; Name: string }[];
  AgentdropdownsingleSettings: {
  ...
  constructor(
    ...
    this.AgentdropdownsingleSettings = {
        ...
        that.http
          .post<DataTablesResponse>(
            this.api.additionParmsEnc(
              environment.apiUrl +
                "/reports/Statement/AgentEarningStatementGridDataNew?User_Id=" +
                this.api.GetUserData("Id") +
                "&User_Code=" +
                this.api.GetUserData("Code") +
                "&User_Type=" +
  ...
  viewSingleStatement(row_Id: any, agentId: any, postingMonth: any) {
    ...
    const dialogRef = this.dialog.open(StatementSingleComponent, dialogConfig);
  ...
  //   let url =
  //     environment.apiUrl + "/reports/Statement/ViewStatementPDF?type=detailed&PO_Id=" + Id;
  //   window.open(url, "", "left=100,top=50,width=800%,height=600");
  // }
  ...
  ViewStatementsPDF(Id): void {
    this.api
      .HttpGetType(
        "/reports/Statement/ViewStatementPDF?type=detailed&PO_Id=" + Id
      )
      .then(
        (result) => {
          if (result["Status"] == true) {
            ...
            window.open(this.Url, "", "left=100,top=50,width=800%,height=600");
          } else {
            this.api.Toast("Warning", result["Message"]);
          }
        },
  ...
  ViewStatements(Id, Agent_Id, Posting_Month_Year, Type): void {
    ...
    if (Type == "PartnerStatement") {
      dialogConfig.data = {
        ...
        SQL_Where: Agent_Id + "|" + Posting_Month_Year + "|detailed",
      };
    } else {
      dialogConfig.data = {
