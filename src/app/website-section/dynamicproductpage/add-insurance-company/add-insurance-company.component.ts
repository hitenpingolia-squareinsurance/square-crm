...
export class AddInsuranceCompanyComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.urlSegment2 == "add_insurance_company") {
      this.id = "";
    } else {
  ...
  getimageValue() {
    ...
    this.api.IsLoading();
    this.api.HttpPostType("WebsiteSection/GetInsuranceCompany", formData).then(
      (result) => {
  ...
  Submit() {
    ...
    if (this.DynamicImageAdd.invalid) {
      ...
      if (this.id != "") {
        formData.append("Id", this.id);
        this.url = "WebsiteSection/EditInsuranceCompany";
      } else {
        this.url = "WebsiteSection/AddInsuranceCompany";
      }
        ...
        (result) => {
          ...
          if (result["status"] == 1) {
            ...
            this.router.navigate([
              "WebsiteSection/products/view_insurance_company",
            ]);
          } else {
            const msg = "msg";
            this.api.Toast("Warning", result["msg"]);
