...
@Component({
  selector: 'app-life-endorsement-track',
  templateUrl: './life-endorsement-track.component.html',
  styleUrls: ['./life-endorsement-track.component.css']
})
export class LifeEndorsementTrackComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<LifeEndorsementTrackComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public api: ApiService, private http: HttpClient, public dialog: MatDialog, private router: Router, private fb: FormBuilder) {
  ...
  Get() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.http
          .post<DataTablesResponse>(environment.apiUrlBms + '/LifeGroup/GridEndorsementData?&User_Id=' + this.api.GetUserData('Id') + '&User_Code=' + this.api.GetUserData('Code') + '&User_Type=' + this.api.GetUserType() + '&SR_Id=' + this.SR_Id, dataTablesParameters, httpOptions
          ).subscribe((res:any) => {
  ...
  UploadBulkExcelComponent(): void {
    ...
    const dialogRef = this.dialog.open(UploadBulkExcelComponent, {
      width: '45%',
      height: '55%',
